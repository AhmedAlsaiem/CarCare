...
import 'package:splash_app/feature/technical/technical_home/presentation/widget/order_in_complate_order.dart';
import 'package:splash_app/feature/technical/technical_home/presentation/widget/shamer_loading.dart';

class OrderListInInprogressOrder extends StatefulWidget {
  final List<OrderEntity> Function(List<OrderEntity>) applyFilter;

  const OrderListInInprogressOrder({super.key, required this.applyFilter});

  @override
  State<OrderListInInprogressOrder> createState() => _OrderListInInprogressOrderState();
}

class _OrderListInInprogressOrderState extends State<OrderListInInprogressOrder> {
    bool _mounted = true;
  ...
  Widget build(BuildContext context) {
    ...
    return current is FaliureOrderState && 
      ...
      listener: (context, state) {
         if (!_mounted) return;
        //  orderHandleState(state, context);
